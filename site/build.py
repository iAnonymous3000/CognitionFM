...
(GitHub Pages: point Pages at the site/ directory).

The public site deliberately excludes docs/04 (channel strategy is internal);
what ships is exactly what a viewer needs to verify our claims: the evidence
review, the playlist designs, the testing method, and the provenance ledger.
"""
...
<title>{"CognitionFM — functional music, honestly designed" if slug == "index" else html.escape(title) + " · CognitionFM"}</title>
...
<footer>Original generative audio, owned end to end. Every mix, image, and claim on this
site is reproducible from the <a href="provenance.html">provenance ledger</a> and the
engine (v{__version__}). No healing frequencies were harmed in the making.</footer>
</body>
</html>
"""
...
downshift, and sleep — generated by our own open engine, with every design decision
traced to published research and every mix reproducible from a seed.</p>

<div class="notice">No 432&nbsp;Hz. No solfeggio. No binaural-beat promises. No
"dopamine hacks." Where the evidence is weak, <a href="evidence.html">we say so</a> —
and for complex verbal work, silence might beat any music, including ours.
<a href="protocol.html">Test it.</a></div>
...
is labeled well-supported, mixed, weak, or debunked — with sources you can click.</li>
...
<h2>Listen</h2>
<p>Mixes and the endless generative stream live on the CognitionFM YouTube channel
<em>(first uploads in preparation)</em>. The engine source, recipes, and these documents
are in the repository <em>(link published with the first upload)</em>.</p>
"""
...
engine version reproduces the exact audio. Cover art derives from the same seed.
This ledger is the ownership record: nothing on the channel samples, remixes, or
